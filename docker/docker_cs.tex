...
\section*{0. Previous Game}
\subsection*{Intalación}

\subsection*{Permisos para crear una imágen}
Al crear una imágen con \textbf{docker build} puede que salte el siguiente error
\begin{center}
    \texttt{Got permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock}
\end{center}
Para resolverlo, debes agregar al usaurio al grupo de DOCKER:
\begin{center}
    \begin{lstlisting}
...
Corrobora con grep docker /etc/group para ver a tu usuario agregado al grupo.
